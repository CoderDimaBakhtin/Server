...
MyServer::MyServer(const uint16_t& PORT){//Creation of the socket and preparation to accept
    ...
    address.sin_addr.s_addr = INADDR_ANY; // get that from user
...
void MyServer::Accept(){
    // Before accepting new client you need to make sure prevoius was closed
...
std::string MyServer::Read(){
    ...
    if (valread == -1) {//Reading Error
        return "-1"; // siignal about an error should be done in separate way
    } else if (valread == 0) {
